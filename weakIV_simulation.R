...
generate_gwas_DP_1 <- function(seed,n,m,theta,K,m0,K0){
  set.seed(seed)
  M = m + m0
  gx = lapply(1:M,function(x){rbinom(n,2,0.3)})
  gx = do.call(cbind,gx)
  bxg = rep(0,M)
  bxg[1:m] = two_runif(m,min1=-0.2,max1=-0.1,min2=0.1,max2=0.2)
  byg = rep(0,M)
  bug = rep(0,M)
  if(K>=1){
...
generate_gwas_DP_2 <- function(seed,n,m,theta,K,m0,K0){
  set.seed(seed)
  M = m + m0
  gx = lapply(1:M,function(x){rbinom(n,2,0.3)})
  gx = do.call(cbind,gx)
  bxg = rep(0,M)
  bxg[1:m] = two_runif(m,min1=-0.1,max1=-0.03,min2=0.1,max2=0.2)
  byg = rep(0,M)
  bug = rep(0,M)
  if(K>=1){
    byg[1:K] = rnorm(K,0.1,0.1)
