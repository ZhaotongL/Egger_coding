...
#Simulation(a) w/ balanced pleiotropy
...
#    bug[1:K] = runif(K,0,b)  ## correlated pleiotropy in Supplementary S8
  }
  eu = rnorm(n,0,1)
  ex = rnorm(n,0,1)
  ey = rnorm(n,0,1)
...
#Simulation(b) w/ balanced pleiotropy
...
#Simulation(a) w/ directional pleiotropy
...
#Simulation(b) w/ directional pleiotropy
...
#Simulation(c) w/ directional pleiotropy
generate_gwas_DP_3 <- function(seed,n,m,theta,K){
  set.seed(seed)
  gx = lapply(1:m,function(x){rbinom(n,2,0.3)})
  gx = do.call(cbind,gx)
  bxg = runif(m,min=0.1,max=0.3)
  byg = rep(0,m)
  bug = rep(0,m)
  if(K>=1){
    byg[1:K] = rnorm(K,0.1,0.1)
...
run_simulation_egger <- function(seed,n,m,theta,K,type){
    ...
    I2_oracle = Isq(b_exp/se_out,se_exp/se_out)
    ...
    I2_default = Isq(b_exp_flip/se_out,se_exp/se_out)
    radial_default = egger_radial(format_radial(b_exp_flip, b_out_flip, se_exp, se_out),summary=FALSE,weights=1)
    ...
    raps = mr.raps.all(b_exp=s$betax,se_exp = s$sdx,b_out = s$betay,se_out = s$sdy)
    raps_pval = 2*pnorm(abs(raps$beta.hat/raps$beta.se),lower.tail=FALSE)
